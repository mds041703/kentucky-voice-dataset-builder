    ...
    Object.values(
        ...
        entry => {
            ...
            return entry.name
                .toLowerCase()
                .startsWith(
                    audioPrefix.toLowerCase()
                );
        }
    );

/*
 * Normal import requires the normal dataset
 * structure. If no usable audio exists, attempt
 * the simple fallback import.
 */
...
/*
 * If the normal import found audio but could
 * not import any recordings, attempt the simple
 * fallback using audio/ and metadata.csv.
 */
...
console.warn(
    "Attempting fallback dataset import using audio/ and metadata.csv."
);

showStatus(
    "Normal import failed. Checking audio/ and metadata.csv..."
);
...
for (
    ...
    /*
     * Fallback assumes all imported audio
     * is WAV regardless of the file extension.
     */
    ...
    if (imported) {
        ...
        console.log(
            `Fallback imported recording: ${audioFile.name}`
        );

    } else {

        console.warn(
            "Dataset rejected fallback imported entry:",
            audioFile.name
        );
    }
}
